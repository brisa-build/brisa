...
const MDX_PATH = path.join(import.meta.dir, 'out', 'cli', 'integrations', 'mdx.js');
const BUILD_PATH = path.join(import.meta.dir, 'out', 'cli', 'build.js');
const SERVE_PATH = path.join(import.meta.dir, 'out', 'cli', 'serve', 'index.js');
...
describe('Brisa CLI', () => {
  beforeEach(() => {
    ...
    mockRandomBytes = spyOn(crypto, 'randomBytes').mockImplementation((bytes) => {
      if (bytes === 32)
        return Buffer.from(
    ...
    mockSpawnSync = spyOn(cp, 'spawnSync').mockImplementation(() => ({ status: 0 }) as any);
  ...
  it('should execute "brisa dev" command with default options', async () => {
    ...
    expect(mockSpawnSync.mock.calls[0]).toEqual(['bun', ['--version'], { stdio: 'ignore' }]);
    expect(mockSpawnSync.mock.calls[1]).toEqual(['bun', [BUILD_PATH, 'DEV'], devOptions]);
    expect(mockSpawnSync.mock.calls[2]).toEqual(['bun', [SERVE_PATH, '3000', 'DEV'], devOptions]);
  });
  ...
  it('should execute "brisa dev" command with custom port', async () => {
    ...
    expect(mockSpawnSync.mock.calls[0]).toEqual(['bun', ['--version'], { stdio: 'ignore' }]);
    expect(mockSpawnSync.mock.calls[1]).toEqual(['bun', [BUILD_PATH, 'DEV'], devOptions]);
    expect(mockSpawnSync.mock.calls[2]).toEqual(['bun', [SERVE_PATH, '5000', 'DEV'], devOptions]);
  });
  ...
  it('should build a desktop app with "brisa dev" command and output=desktop', async () => {
    ...
    expect(mockSpawnSync.mock.calls[3]).toEqual(['bun', [BUILD_PATH, 'DEV'], devOptions]);
  ...
  it('should build a desktop app with "brisa build" command and output=desktop', async () => {
    ...
    expect(mockSpawnSync.mock.calls[3]).toEqual(['bun', [BUILD_PATH, 'PROD'], prodOptions]);

    expect(mockSpawnSync.mock.calls[4]).toEqual(['bunx', ['tauri', 'build'], prodOptions]);
  });
  ...
  it('should build a ios app with "brisa build" command and output=ios', async () => {
    ...
    expect(mockSpawnSync.mock.calls[4]).toEqual(['bunx', ['tauri', 'ios', 'build'], prodOptions]);
  });
  ...
  it('should execute "brisa start" command with default options', async () => {
    ...
    expect(mockSpawnSync).toHaveBeenCalledTimes(2); // bun --version
    expect(mockSpawnSync.mock.calls[0]).toEqual(['bun', ['--version'], { stdio: 'ignore' }]);
    expect(mockSpawnSync.mock.calls[1]).toEqual(['bun', [SERVE_PATH, '3000', 'PROD'], prodOptions]);
  });
  ...
  it('should execute "brisa start" command with custom port', async () => {
    ...
    expect(mockSpawnSync).toHaveBeenCalledTimes(2); // bun --version
    expect(mockSpawnSync.mock.calls[0]).toEqual(['bun', ['--version'], { stdio: 'ignore' }]);
    expect(mockSpawnSync.mock.calls[1]).toEqual(['bun', [SERVE_PATH, '5000', 'PROD'], prodOptions]);
  });

  it('should execute .bun/bin/bun when the bun command is not found', async () => {
    mockSpawnSync = spyOn(cp, 'spawnSync').mockImplementation(() => ({ status: 1 }) as any);
    ...
    expect(mockSpawnSync.mock.calls[1][0]).toBe(path.join(process.env.HOME!, '.bun', 'bin', 'bun'));
    expect(mockSpawnSync.mock.calls[1]).toEqual([
  ...
  it('should "brisa add mdx" command integrate MDX', async () => {
    ...
    expect(mockSpawnSync.mock.calls[2]).toEqual(['bun', [MDX_PATH], devOptions]);
  });
