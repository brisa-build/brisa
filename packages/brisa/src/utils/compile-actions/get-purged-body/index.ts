...
export function getPurgedBody(info: ActionInfo): ESTree.BlockStatement {
  ...
  const body = (info.componentFnExpression?.body ?? defaultBody) as ESTree.BlockStatement;
  const bodyIdentifiers = getVarDeclarationIdentifiers(body);
  const actionFn = info.actionFnExpression ?? getActionFnFromActionIdentifier(info);
  ...
  return {
    ...
    body: removeAllReturns(body.body ?? [body]).filter((node) => {
      return (node as any).__isActionFn || containsIdentifiers(node, intersectionSet);
    }),
  };
}

function getActionFnFromActionIdentifier(info: ActionInfo) {
  let actionFn: ESTree.FunctionExpression | ESTree.ArrowFunctionExpression | undefined;

  JSON.stringify(info.componentFnExpression, (k, v) => {
    if (info.actionIdentifierName && info.actionIdentifierName === v?.id?.name) {
      v.__isActionFn = true;
      actionFn = v;
    }
    return v;
