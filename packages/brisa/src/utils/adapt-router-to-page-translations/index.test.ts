  ...
  describe('adaptRouterToPageTranslations', () => {
    describe('given a real router', () => {
      it('should NOT return the route given the original pathname respecting the locale page', () => {
        const { match, reservedRoutes } = adaptRouterToPageTranslations(pages, router);
        const { route, isReservedPathname } = match(createRequest('https://example.com/somepage'));
      ...
      it('should return the dynamic route given the translated pathname', () => {
        ...
        expect(route).not.toBe(null);
        expect(route?.filePath).toBe(path.join(PAGES_DIR, 'user', '[username].tsx'));
        expect(isReservedPathname).toBe(false);
        expect(reservedRoutes['/_404']).toBeDefined();
      });
    });
    ...
    describe('given a mock router', () => {
      it('should detect all the translated routes', () => {
        ...
        const { match } = adaptRouterToPageTranslations(mockPages, mockRouter as any);

        const getPathname = (v: any) => match(createRequest(v))?.route?.pathname;

        expect(getPathname('https://example.com/alguna-pagina')).toBe('/somepage');
        expect(getPathname('https://example.com/usuario/aral')).toBe('/user/aral');
        expect(getPathname('https://example.com/algo/1/2/3')).toBe('/some/1/2/3');
        expect(getPathname('https://example.com/otra-cosa/1/2/3')).toBe('/another/1/2/3');
        expect(getPathname('https://example.com/')).toBe('/');
        expect(getPathname('https://example.com/usuario/aral/configuracion/1')).toBe(
          '/user/aral/settings/1',
        );

        // not untranslated because is not in the current locale "es"
        expect(getPathname('https://example.com/qualsiasi-pagina')).toBe('/qualsiasi-pagina');
      });
      ...
      it('should detect all the translated routes with locale prefix', () => {
        ...
        expect(getPathname('https://example.com/es/alguna-pagina')).toBe('/somepage');
        expect(getPathname('https://example.com/es/usuario/aral')).toBe('/user/aral');
        expect(getPathname('https://example.com/es/algo/1/2/3')).toBe('/some/1/2/3');
        expect(getPathname('https://example.com/es/otra-cosa/1/2/3')).toBe('/another/1/2/3');
        expect(getPathname('https://example.com/es')).toBe('/');
        expect(getPathname('https://example.com/es/usuario/aral/configuracion/1')).toBe(
          '/user/aral/settings/1',
        );

        // not untranslated because is not in the current locale "es"
        expect(getPathname('https://example.com/es/qualsiasi-pagina')).toBe('/qualsiasi-pagina');
      });
      ...
      it('should detect all the translated routes with trailingSlash', () => {
        ...
        // not untranslated because is not in the current locale "es"
        expect(getPathname('https://example.com/es/qualsiasi-pagina/')).toBe('/qualsiasi-pagina');
        expect(getPathname('https://example.com/es/somepage-without-spanish/')).toBe(
          '/somepage-without-spanish',
        );
      });
    });
  });
