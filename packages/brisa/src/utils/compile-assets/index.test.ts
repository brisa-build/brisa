...
import { describe, it, beforeEach, afterEach, beforeAll, afterAll, expect } from 'bun:test';
...
describe('compileAssets', () => {
  ...
  it('should compile fixtures assets correctly', async () => {
    await compileAssets();
    expect(fs.readdirSync(BUILD_DIR).toSorted()).toEqual(['public', 'pages-client'].toSorted());
    expect(fs.readdirSync(path.join(BUILD_DIR, 'public')).toSorted()).toEqual(
      ['favicon.ico', 'favicon.ico.br', 'favicon.ico.gz', 'some-dir'].toSorted(),
    );
    expect(fs.readdirSync(path.join(BUILD_DIR, 'public', 'some-dir')).toSorted()).toEqual(
      [
        'some-text.txt.br',
        'some-img.png.br',
        'some-text.txt.gz',
