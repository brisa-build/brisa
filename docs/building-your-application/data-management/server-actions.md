...
- `X-Target`: Used for [`renderComponent()`](/api-reference/server-apis/renderComponent) with a different `target` to render.
- `X-Placement`: This header is used to determine the `placement` on the `target` when using [`renderComponent()`](/api-reference/server-apis/renderComponent).
- `X-Navigate`: This header indicates that instead of observing the stream, the client should navigate to another route.
- `X-Cid`: This header returns the id of the component that called the [`renderPage`](#renderpage-and-rendercomponent) method with `component` type. This header is used internally by Brisa to determine which component should be rerendered. When it is not present, the component that fired the action is rerendered.
